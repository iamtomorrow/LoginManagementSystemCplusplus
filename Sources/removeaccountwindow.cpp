...
#include "ui_removeaccountwindow.h"
...
// functions;
bool usernameConfirmationRemoveAccount(QString un);
bool passwordConfirmationRemoveAccount(QString password);
bool passwordReconfirmationRemoveAccount(QString password, QString passwordAgain);


RemoveAccountWindow::RemoveAccountWindow(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::RemoveAccountWindow)
{
    ui->setupUi(this);
    ...
    this->setWindowTitle("Remove Account");
...
void RemoveAccountWindow::on_minimizeButton_clicked()
{
    showMinimized();
    minimized = true;
    maximized = false;
...
void RemoveAccountWindow::on_maximizaButton_clicked()
{
    showMaximized();
    minimized = false;
    maximized = true;
...
void RemoveAccountWindow::on_normalButton_clicked()
{
    showNormal();
    minimized = false;
    maximized = false;
...
void RemoveAccountWindow::on_closeButton_clicked()
{
    db.close();
    close();
}
...
void RemoveAccountWindow::on_menuButton_clicked()
{
    if (menu) {
        menu = false;
...
void RemoveAccountWindow::on_toggleButton_clicked()
{
    if (!toggle) {
        toggle = true;
        ui->centralwidget->setStyleSheet("border-radius: 10px; background-color: qlineargradient(spread:pad, x1:0.477169, y1:0.505, x2:0.753, y2:0.608091, stop:0 rgba(0, 66, 126, 255), stop:1 rgba(0, 86, 151, 255));");
...
void RemoveAccountWindow::on_aboutOption_clicked()
{
...
void RemoveAccountWindow::on_exitOption_clicked()
{
...
void RemoveAccountWindow::on_signupOption_clicked()
{
    SignUpWindow SignUp;
    this->close();
    SignUp.exec();
...
void RemoveAccountWindow::on_signinOption_clicked()
{
    SignInWindow SignIn;
    this->close();
    SignIn.exec();
...
void RemoveAccountWindow::on_visibilityButton_clicked()
{
    if (echoModeActive) {
...
void RemoveAccountWindow::on_removeAccountButton_clicked()
{
    ...
    if (passwordConfirmationRemoveAccount(userPassword) && passwordReconfirmationRemoveAccount(userPassword, userPasswordConfirmed)) {
    ...
    ans = QMessageBox::question(this, "Account Remotion Initiated", "Are you sure want to remove you account? The process of account remotion does not save any user data, meaning that all of your information will be deleted forever.", QMessageBox::Yes | QMessageBox::No);
    if (ans == QMessageBox::Yes) {
        if (db.open()) {
            ...
            if (query.exec()) {
                ...
                if (count == 1) {
                    ...
                    if (query.exec()) {
                        QMessageBox::warning(this, "SUCCESSFUL removed", "User was SUCCESSFUL removed! ");
                    }
                    else {
                        // 001 execution error;
                        QMessageBox::warning(this, "Execution ERROR", "ERROR trying to execute the deletion of the user! Please consider contact us for possible solutions, since this problem does not belong to expected errors.");
                        qDebug() << "ERROR trying to delete user in our database!";
        ...
        QMessageBox::warning(this, "Account Remotion Abort", "You decided mantain your account active! We are very happy.");
    }
}
